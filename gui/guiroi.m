function rois = guiroi(axroi, kwargs)
    ...
    arguments (Input)
        ...
        kwargs.snap (1,:) = []
        kwargs.position {mustBeA(kwargs.position, {'double', 'cell'})} = [] % two row vertex to line selection; edge size to rectangle selection; n-row verxex to polygon selection 
        kwargs.interaction (1,:) char {mustBeMember(kwargs.interaction, {'all', 'none', 'translate'})} = 'all' % region selection behaviour
        kwargs.number (1,:) {mustBeInteger, mustBeGreaterThan(kwargs.number, 0)} = 1 % count of selection regions
